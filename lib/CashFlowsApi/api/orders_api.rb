...
module CashFlowsApi
  class OrdersApi
    ...
    # @param order_number The number of the order to check.
    # @param configuration_id Your configuration ID.
    # @param hash A hexadecimal sha512 hash of your password + the body.
    # @param [Hash] opts the optional parameters
    # @return [nil]
    def api_gateway_orders_order_number_head(order_number, configuration_id, hash, opts = {})
      api_gateway_orders_order_number_head_with_http_info(order_number, configuration_id, hash, opts)
      nil
    end
    ...
    def api_gateway_orders_order_number_head_with_http_info(order_number, configuration_id, hash, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: OrdersApi.api_gateway_orders_order_number_head ...'
      end
      # verify the required parameter 'order_number' is set
      if @api_client.config.client_side_validation && order_number.nil?
        fail ArgumentError, "Missing the required parameter 'order_number' when calling OrdersApi.api_gateway_orders_order_number_head"
      end
      # verify the required parameter 'configuration_id' is set
      if @api_client.config.client_side_validation && configuration_id.nil?
        fail ArgumentError, "Missing the required parameter 'configuration_id' when calling OrdersApi.api_gateway_orders_order_number_head"
      end
      # verify the required parameter 'hash' is set
      if @api_client.config.client_side_validation && hash.nil?
        fail ArgumentError, "Missing the required parameter 'hash' when calling OrdersApi.api_gateway_orders_order_number_head"
      end
      # resource path
      local_var_path = '/api/gateway/orders/{orderNumber}'.sub('{' + 'orderNumber' + '}', order_number.to_s)
      ...
      header_params[:'configurationId'] = configuration_id
      header_params[:'hash'] = hash
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: OrdersApi#api_gateway_orders_order_number_head\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
