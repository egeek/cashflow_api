  ...
  class CardTokenisationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::CardTokenisationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::CardTokenisationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @card_expiry_year.nil?
        invalid_properties.push('invalid value for "card_expiry_year", card_expiry_year cannot be nil.')
      end

      if @card_expiry_month.nil?
        invalid_properties.push('invalid value for "card_expiry_month", card_expiry_month cannot be nil.')
      end

      if @card_cvc.nil?
        invalid_properties.push('invalid value for "card_cvc", card_cvc cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @card_expiry_year.nil?
      return false if @card_expiry_month.nil?
      return false if @card_cvc.nil?
      true
    end
    ...
    def hash
      [card_number, card_holder_name, card_expiry_year, card_expiry_month, card_cvc].hash
    end
