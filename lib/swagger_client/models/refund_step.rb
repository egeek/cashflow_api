  ...
  class RefundStep
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::RefundStep` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::RefundStep`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'result_attributes')
        if (value = attributes[:'result_attributes']).is_a?(Hash)
          self.result_attributes = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          result_attributes == o.result_attributes
    end
    ...
    def hash
      [reference, create_date_time_utc, action, status, description, result_attributes].hash
    end
